    ...
    <ScrollView style={s.root}>
      ...
      {editProfile&&(
        <View style={s.formCard}>
          ...
          {[['FIRST NAME',form.firstName,v=>setForm(f=>({...f,firstName:v}))],['LAST NAME',form.lastName,v=>setForm(f=>({...f,lastName:v}))],['EMAIL',form.email,v=>setForm(f=>({...f,email:v}))],['EMPLOYER',form.employer,v=>setForm(f=>({...f,employer:v}))]].map(([l,v,fn])=>(
      ...
      {changePwd&&(
        <View style={s.formCard}>
          ...
          {[['CURRENT PASSWORD',pwdForm.current,v=>setPwdForm(f=>({...f,current:v}))],['NEW PASSWORD (min 8)',pwdForm.next,v=>setPwdForm(f=>({...f,next:v}))],['CONFIRM NEW',pwdForm.confirm,v=>setPwdForm(f=>({...f,confirm:v}))]].map(([l,v,fn])=>(
      ...
      <View style={s.section}>
        ...
        {[['🗄','Storage','SQLite (local) + IndexedDB (cloud)'],['🔐','Security','bcrypt SHA-256 · JWT · WebAuthn'],['💱','Live Data','Frankfurter API (ECB) · Free tier'],['📷','OCR','Google ML Kit · On-device · Free'],['📄','Exports','jsPDF · SheetJS · Free']].map(([i,t,s])=>(
          <View key={t as string} style={s.row}>
            <Text style={s.rowIcon}>{i as string}</Text>
            <View style={{flex:1}}><Text style={s.rowTitle}>{t as string}</Text><Text style={s.rowSub}>{s as string}</Text></View>
          </View>
        ))}
      </View>
