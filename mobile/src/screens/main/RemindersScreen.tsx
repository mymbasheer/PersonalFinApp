...
export default function RemindersScreen() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (dl <= 3) await Notifications.scheduleReminder(String(Date.now()), `Reminder: ${form.title}`, `Due: ${form.dueDate}${Number(form.amount) > 0 ? ` · LKR ${fmt(Number(form.amount))}` : ''}`, form.dueDate);
    ...
    return (
      <View style={[s.remCard, { borderLeftColor: rem.color || COLORS.gold }]}>
        ...
        <View style={s.remBottom}>
          ...
          <TouchableOpacity onPress={() => Alert.alert('Remove?', '', [{ text: 'Cancel', style: 'cancel' }, { text: 'Remove', style: 'destructive', onPress: async () => { await api.delete(`/reminders/${rem.id}`); load(); } }])} style={s.xBtn}>
